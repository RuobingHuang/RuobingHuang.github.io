...
function normalizeAuthors(authorField) {
  ...
  return authorField.split(/\s+and\s+/i).map(s => s.trim()).join(", ");
}
...
function asBibtexBlock(e) {
  // Keep a simple stable bibtex output for copying (not perfect formatting, but good enough)
  const key = e.citationKey || "unknown";
  const type = (e.entryType || "article").toLowerCase();
  const tags = e.entryTags || {};
  const order = ["title","author","journal","booktitle","year","pages","publisher","doi","url"];
  const lines = [];
...
const yearSections = years.map(y => {
  const items = byYear.get(y).map(e => {
    ...
    return `
      <div class="pub">
        <div class="pub-title">${escHtml(title || "(untitled)")}</div>
        ...
        <details style="margin-top:10px;">
          ...
          <div style="margin-top:10px;">
            <button class="btn" data-copy-bibtex="bib-${escHtml(key)}">Copy BibTeX</button>
            <pre class="bibtex" id="bib-${escHtml(key)}">${escHtml(bib)}</pre>
          </div>
        </details>
      </div>
    `.trim();
...
<main>
  ...
  <div class="card">
    ${yearSections || "<p class=\\"small\\">No publications found in publications.bib</p>"}
  </div>
